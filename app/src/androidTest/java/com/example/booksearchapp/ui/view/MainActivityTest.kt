...
//          2-2) 리사이클러뷰 표시를 확인
        onView(withId(R.id.rv_favorite_books)).check(matches(isDisplayed()))
//          2-3) 첫번째 아이템을 슬라이드하여 삭제
    ...
    private fun waitFor(dealy: Long): ViewAction {
        return object : ViewAction {
            override fun getDescription(): String = "wait for $dealy milliseconds"

            override fun getConstraints(): Matcher<View> = isRoot()

            override fun perform(uiController: UiController, view: View?) {
                uiController.loopMainThreadForAtLeast(dealy)
            }
